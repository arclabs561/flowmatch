//! Riemannian Flow Matching on the Poincare ball.
//!
//! Trains a tangent-space linear vector field to transport a base distribution (small ball
//! near the origin) to a set of target points on the Poincare disk (2D, curvature c=1).
//! After training, samples are generated by integrating the learned field with both Euler
//! and Heun methods and the results are compared.
//!
...
fn main() {
    ...
    let targets: Vec<[f32; 2]> = vec![[0.4, 0.3], [-0.3, 0.5], [-0.4, -0.2], [0.2, -0.5]];
    ...
    // -- Learnable vector field: tangent-space linear map W*[x;t] + b --
    // Input is (dim + 1) because we concatenate (x, t). Output is dim.
    const DIM: usize = 2;
    const INPUT: usize = DIM + 1; // [x0, x1, t]

    struct LinearTangentField {
        w: [[f32; INPUT]; DIM], // weight matrix (DIM x INPUT)
        b: [f32; DIM],          // bias
    }

    impl LinearTangentField {
        fn new() -> Self {
            // Small nonzero init so gradients flow from the start.
            Self {
                w: [[0.01, -0.01, 0.02], [0.01, 0.02, -0.01]],
                b: [0.0; DIM],
            }
        }
    }
    ...
    impl ManifoldVectorField for LinearTangentField {
        ...
        fn sgd_step(&mut self, x: &ArrayView1<f32>, t: f32, target: &ArrayView1<f32>, lr: f32) {
            let pred = self.eval(x, t);
            // Gradient of L2 loss: 2 * (pred - target). We absorb the factor of 2 into lr.
            let mut input = [0.0f32; INPUT];
            for j in 0..DIM {
                input[j] = x[j];
            }
            input[DIM] = t;
            ...
            for i in 0..DIM {
                let grad_out = pred[i] - target[i];
                for j in 0..INPUT {
                    self.w[i][j] -= lr * grad_out * input[j];
                }
                self.b[i] -= lr * grad_out;
            }
        }
    }
    ...
    println!(
        "Training complete ({} steps, batch_size {})",
    ...
    // Sanity: with a linear model on 4 targets, we expect most points inside the ball.
    assert!(
        euler_in == n_samples,
